...
// 调试接口
void adc_get_offset(adc_offset_t *offsets);     // 获取ADC零点偏移
uint32_t adc_get_injected_irq_count(void);      // 注入组中断触发计数器
uint32_t adc_get_injected_callback_count(void); // 注入组用户ADC回调执行计数器, 触发中断不一定执行回调
