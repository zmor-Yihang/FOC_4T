...
void foc_init(foc_t *handle, pid_controller_t *pid_id, pid_controller_t *pid_iq, pid_controller_t *pid_speed)
{
    ...
    /* 初始化弱磁控制器 */
    // 弱磁电流限制，防止永磁体退磁
    flux_weak_init(&handle->flux_weak, U_DC, 0.9f, 0.005f, -1.5f);
}
...
void foc_open_loop_run(foc_t *handle, float speed_rpm, float voltage_q)
{
     ...
     */
    const float control_dt = 0.0001f;
    float delta_angle = 2.0f * M_PI * AS5047_MOTOR_POLE_PAIR * (speed_rpm / 60.0f) * control_dt;
