...
{
    // 计算角度
    float angle_el = as5047_get_angle_rad() - foc_current_closed_handle.angle_offset;
    ...
    i_dq_temp = i_dq;
    as5047_update_speed();
    speed_temp = as5047_get_speed_rpm_lpf();
...
{
    // 初始化电流环 PID 控制器
    pid_init(&pid_id, 0.179f, 0.167f, -U_DC / 2.0f, U_DC / 2.0f);
    pid_init(&pid_iq, 0.179f, 0.167f, -U_DC / 2.0f, U_DC / 2.0f);
    ...
    // 注册回调函数
    adc1_register_injected_callback(current_closed_callback);
}

void print_current_info(void)
{
    float data[3] = {i_dq_temp.d, i_dq_temp.q, speed_temp};
    printf_vofa(data, 3);
}